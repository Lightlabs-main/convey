...
Status: **active at the account-abstraction transaction gate.** Product-contract work remains sequenced after one sponsored mainnet UserOperation succeeds.
...
- Bytecode exists at the canonical ERC-4337 EntryPoint v0.6, v0.7, and v0.8 addresses. This proves deployment, not bundler support; Ticker is pinned to v0.7 for the selected OKX wallet path.
...
- The local private bundler runtime probe passed. A persistent operator endpoint, deployed paymaster, escrow address/claim selector, and funded paymaster deposit/stake are not configured yet.
...
Contract implementation must not begin until a self-hosted bundler/paymaster deployment is live and one sponsored mainnet UserOperation succeeds.
